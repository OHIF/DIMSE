...
DIMSE.associate = (contexts, callback, options) => {
  ...
  const socket = this.connection.associate(options, (pdu) => {
    // Associated
...
DIMSE.retrievePatients = (params, options) => {
  ...
  DIMSE.associate([constants.SOP_PATIENT_ROOT_FIND], (error, pdu) => {
    if (error) {
...
DIMSE.retrieveStudies = (params, options) => {
  ...
  DIMSE.associate([constants.SOP_STUDY_ROOT_FIND], (error, pdu) => {
    if (error) {
...
DIMSE._retrieveInstancesBySeries = (connection, series, studyInstanceUID, callback, params) {
  ...
  let result = connection.findInstances(Object.assign(defaultParams, params)),
    instances = [];
...
DIMSE.retrieveInstancesByStudyOnly = (studyInstanceUID, params, options) => {
  ...
  DIMSE.associate([constants.SOP_STUDY_ROOT_FIND], (error, pdu) => {
    ...
    result.on('end', () => {
      if (series.length > 0) {
        DIMSE._retrieveInstancesBySeries(self, series, studyInstanceUID, (relatedInstances, isEnd) => {
...
DIMSE.storeInstances = (fileList, callback) => {
  const handle = this.connection.storeInstances(fileList);
...
DIMSE.moveInstances = (studyInstanceUID, seriesInstanceUID, sopInstanceUID, sopClassUID, params) => {
  DIMSE.associate([constants.SOP_STUDY_ROOT_MOVE, sopClassUID], (error) => {
    if (error) {
